...
class CustomerController {
    ...
    try {
      if (req.query?.ND_id) {
        const customers = await queryMysql(
          `select ND_id, ND_email, ND_ten, ND_gioitinh, ND_SDT, ND_diaChi from nguoi_dung where ND_id=${req.query.ND_id}`
        );
  ...
  async updateAddress(req, res) {
    try {
      if (req.body?.address) {
        const update_address = await queryMysql(
          `update nguoi_dung set ND_diaChi='${req.body.address}' where ND_id=${req.body.ND_id}`
        );
